...
def check_all_listings_and_mail(
    ...
    all_available = db.query(EbayListing).filter(EbayListing.status == ListingStatus.AVAILABLE).all()
    ...
    for listing in all_available:
        try:
            ...
            if listing.has_changed_price(float(price)):
                ...
                logger.info("Sent email for listing", listing_id=listing.id, old_price=listing.entry_price, new_price=float(price))


                listing.entry_price = float(price)
                db.commit()
                db.refresh(listing)
                logger.info("Updated listing price in database", listing_id=listing.id, new_price=float(price))
                
        except ValidationError as e:
            logger.error("Validation error while processing listing", listing_id=listing.id, error=str(e))
            listing.status = ListingStatus.SOLD
            db.commit()
            db.refresh(listing)
            logger.info("Updated listing status to SOLD due to validation error", listing_id=listing.id)
        except Exception as e:
            logger.error("Unexpected error while processing listing", listing_id=listing.id, error=str(e))
